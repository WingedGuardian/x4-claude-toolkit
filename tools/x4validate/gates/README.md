...
> **`mutation_probe.py` ~70 s** (48 mutants across 9 files; a survivor costs one extra
...
<!-- A table header must ADJOIN its rows. This one sat 30 lines above them with a
     `## Running them` section in between, so GitHub rendered the section as prose
...
| `schema_sweep.py` | `uv run python gates/schema_sweep.py` | **178 pairs · 77 gating + 71 advisory + 3 suppressed · 1 NOT checked · 41 mods flagged** (re-measured 2026-09-06 by running the gate; the previous row read 168/59/70/42, of which the pairs and mods-flagged figures were already stale and the gating/advisory pair was made stale by this release's own re-baseline), and the independently-evidenced defects still reported. Freezes the composition of the output so it cannot drift unnoticed. *(Baseline history lives in the measurement tables above the constants — most recently 2026-09-06, where the move was again attributed per-mod rather than assumed: three added mods contribute 1 gating + 2 advisory, and one removed mod took 2 gating + 9 advisory with it, so the arithmetic closes exactly. **`NOT checked` is pinned too**: the gate previously froze only what WAS checked, so 31 documents skipped with a false reason never moved a number here.)* |
...
| `routing_coverage.py` | `uv run python gates/routing_coverage.py` | **every CLI in `[project.scripts]` appears in CLAUDE.md's routing-table `Use` column.** MEASURED 2026-09-12: **5 of 11** were absent — `x4modlist`, `x4stats`, `x4diff`, `x4save`, `x4live` — through a full CLAUDE.md rewrite, because whether a tool is routed was a PROSE property nothing tested. ⚠ **It claims RESIDENT ROUTING, not discoverability**: a cold agent found an unrouted CLI and used it correctly in 193 s via KNOWLEDGEBASE.md and the README, so the cost of a missing row is SEARCH (39 tool calls, ~74k tokens), not unreachability. Checks BOTH the shipped `CLAUDE.md` (repo root — in every clone) and the configured game root's copy; the first version read only the latter and was green solely because the author's personal copy had been hand-edited. The surface is an explicit constant, never inferred — when the inventory moves to a generated registry, change it deliberately. Subcommand coverage is INFO, never gated (uncalibrated; a gate that floods is worse than none). Enumeration comes from `x4validate/_surface.py`, which ASKS the programs: a static `add_parser` scan finds 39 of 43. |
| `claude_md_budget.py` | `uv run python gates/claude_md_budget.py` [`--record`] | **no CLAUDE.md exceeds its recorded floor.** A RATCHET, not a ceiling: "under 40,000 chars" reports PASS on a file full of content that has a better home. MEASURED 2026-09-12, the two files moved in OPPOSITE directions — game root 154,425 → 38,056 (−75%) while the SHIPPED one went 18,420 → 70,118 in ONE commit (`8289d5a`, +624 lines, unannounced). This would not have blocked that commit; it would have made it name what it displaced. ⚠ Counts CONTENT characters: from the bytes, with each CRLF counted as ONE newline. Counting the raw CR made a CRLF checkout of an unchanged file read as GREW +872 (F120, 2026-09-13); mixing a CRLF baseline with `read_text()` is lax by the line count. Paths come from `__file__`, never `$X4_TOOLKIT` — that variable had two live values on one machine at once. Baseline is LOCAL and gitignored; absent is reported as "drift is NOT being checked", never as a pass. |
| `deploy_parity.py` | `uv run python gates/deploy_parity.py` | **a configured game root's `.claude/` equals this repo's** -- the repo is the source, the game root's copy is a deployment. MEASURED 2026-09-13: **5 of 25** deployed files had drifted in three directions (two skills written straight into the deployment and never shipped, one skill newer there, two agents hand-edited with personal paths). CRLF is not drift; the installer's `$CLAUDE_PROJECT_DIR` -> `$X4_TOOLKIT` rewrite is not drift **for `skills/` and `agents/` only**, exactly as install.sh applies it. Reports DIRECTION per file and names no winner -- `scripts/deploy-claude-dir.py` acts, and refuses to overwrite a never-ported edit. ⚠ **No accept-baseline**: dev's predecessor `published_surface_drift.py` had one, and accepted drift is how this drift lasted. rc 2 with no game root (never "at parity"). |
| `instrument_hygiene.py` | `uv run python gates/instrument_hygiene.py [--record] [--transcripts DIR]` | **no known-bad shell SHAPE got worse or appeared**, counted over THIS machine's session transcripts -- `$?` after a pipe, an inline-interpreter heredoc, `grep -c` used as a count, a diff against a hand-rolled baseline, and the rest of its table. A LOWER BOUND, and it says so: a regex sees shapes, never a wrong population. **Every pattern carries a fixture it must match and a near-miss it must not**, checked before any count is produced -- the file's own first backslash pattern was wrong three times in ten minutes. Each shape's share of all commands is printed as the evidence for whether it could ever be a PreToolUse rule. Baseline gitignored (per-machine transcripts); rc 2 without transcripts. |
| `hook_false_positives.py` | `uv run python gates/hook_false_positives.py [--record] [--limit=N] [--workers=N]` | ~35 min, so `run-gates.sh` quick mode skips it. **no per-rule drift in `protect-bash.sh`'s verdicts** when every historical Bash command in THIS machine's transcripts is replayed through the deployed hook. Each of those commands ran fine when issued, so a deny or advise is the guard's own verdict on work that needed doing. **Controls first**: it refuses unless a known-bad command comes back `deny` and a harmless one `allow` (a first attempt scored a vacuous 300/300 allow because the replay environment had no `X4_TOOLKIT`). Hashes the hook files before and after and VOIDS a run if they moved. Compares the INTERSECTION of commands present in both runs, so a growing corpus is never a rule change. rc 1 drift, 2 refused, 3 unexpected exception. Baseline gitignored. |
| `register_rederivation.py` | `uv run python gates/register_rederivation.py [--record]` | **every FIXED/CLOSED entry in `docs/BLIND-SPOTS.md` names a check that EXISTS** -- a test, gate or suite path, a bare test name defined under `tests/` or `.claude/hooks/`, or an explicit `NO RE-DERIVATION` with its reason. A fix that is only prose decays, and an entry naming a deleted test is worse than one naming nothing, because it reads as covered. The known backlog is a committed baseline, NAMED and printed every run; only a NEW unnamed entry fails. Citations resolve at the package root and at the repo root that nests it, including `tools/basex/test_*.py` (a bare test name defined only there counts when the entry cites that file), and a cited path must START at a path boundary -- `mytools/basex/test_ask.py` is not a citation of `tools/basex/test_ask.py`. |
