...
| `claude_md_budget.py` | `uv run python gates/claude_md_budget.py` [`--record`] | **no CLAUDE.md exceeds its recorded floor.** A RATCHET, not a ceiling: "under 40,000 chars" reports PASS on a file full of content that has a better home. MEASURED 2026-09-12, the two files moved in OPPOSITE directions — game root 154,425 → 38,056 (−75%) while the SHIPPED one went 18,420 → 70,118 in ONE commit (`8289d5a`, +624 lines, unannounced). This would not have blocked that commit; it would have made it name what it displaced. ⚠ Counts CHARACTERS from `read_bytes().decode()` — `read_text()` applies universal newlines and deletes one char per line, so a ratchet built on it is lax by the LINE COUNT and gets laxer as the file grows (38,056 vs 37,460 on the real file, a gap of exactly its 596 CRLF pairs). Paths come from `__file__`, never `$X4_TOOLKIT` — that variable had two live values on one machine at once. Baseline is LOCAL and gitignored; absent is reported as "drift is NOT being checked", never as a pass. |
