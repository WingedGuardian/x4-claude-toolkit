...
- **Load order is NOT encoded in `content.xml`** (it lists `id` + `enabled` only). Tiering: base → 6 DLC → enabled mods. **Inter-mod order is undocumented** — any tool wanting bit-exact multi-mod fidelity must confirm empirically (dump the game's merged XML). DLC diffs use defensive `if="not(...)"` guards, implying DLC-applied-before-mods and order-independence within DLC.
- **Active user profile = `{profile-id}`** (`Documents\Egosoft\X4\{profile-id}\`); `213512290` is stale (2023). `content.xml` there: ~289 extensions, ~97 enabled. Enabled Workshop mods live in the Steam Workshop content folder, not the game-root `extensions\` (which holds only DLC + the 3 dev mods).
